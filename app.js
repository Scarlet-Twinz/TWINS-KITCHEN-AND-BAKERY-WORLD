...
if(!signup&&user){location.href=(user.remote&&user.role==='owner')?'admin/media/':'dashboard.html';return}
...
try{var remote=await apiRequest('/api/auth/login',{method:'POST',body:JSON.stringify({email:email,password:pass})});put('twins_user',{id:remote.user.id,name:remote.user.name,email:remote.user.email,remote:true,role:remote.user.role});localStorage.removeItem('twins_demo_auth');location.href=remote.user.role==='owner'?'admin/media/':'dashboard.html';return}catch(e){if(!isNetworkError(e)){toast(e.message||'Sign in failed');return}}
