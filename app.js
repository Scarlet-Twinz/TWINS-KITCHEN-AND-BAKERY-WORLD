...
function planMarkup(business){
 ...
 "<div class='grid builderproducts'>"+products.map(function(p){return "<label class='card buildercard'><input type='checkbox' name='equipmentIds' value='"+p.id+"' "+checked(p.id)+"><div class='cardbody'><span class='eyebrow darkey'>"+esc(p.c)+"</span><h3>"+esc(p.n||"Equipment")+"</h3><p>"+esc(p.desc||"Commercial equipment reference.")+"</p><span class='muted small'>ID "+p.id+" · Quote on request</span></div></label>";}).join("")+"</div>";
}
