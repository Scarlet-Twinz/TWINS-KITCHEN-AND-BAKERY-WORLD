...
function demoAuthAllowed(){return typeof location!=="undefined"&&/^(localhost|127\.0\.0\.1)$/.test(location.hostname)}
...
if(!demoAuthAllowed()){toast('Account service is unavailable. Please try again later.');return}var passwordHash=await hashPassword(pass);put('twins_user',{name:name.trim(),email:email,remote:false});put('twins_demo_auth',{email:email,passwordHash:passwordHash});location.href='dashboard.html';
}else{
...
if(!demoAuthAllowed()){toast('Account service is unavailable. Please try again later.');return}var savedUser=get('twins_user',null),demo=get('twins_demo_auth',null),passwordHash=await hashPassword(pass);
...
var savedPlan=get('twins_business_plan',null);
var requested=params.get('type')||'';
var selected=(savedPlan&&savedPlan.type)||requested||'';
if(!B.some(function(x){return x[0]===selected}))selected='';
var selectedBusiness=B.find(function(x){return x[0]===selected;})||null;
var businessOptions=B.map(function(x){return '<option value="'+String(x[0]).replace(/"/g,'&quot;')+'" '+(x[0]===selected?'selected':'')+'>'+x[0]+'</option>';}).join('');
var cats=selectedBusiness?selectedBusiness.slice(2):[];
var recs=cats.length?P.filter(function(p){return cats.indexOf(p.c)>-1}).slice(0,18):P.slice(0,18);
var selectedIds=(savedPlan&&savedPlan.equipmentIds)||[];
var checked=function(id){return selectedIds.indexOf(Number(id))>-1?'checked':''};
function planMarkup(business){
 var b=B.find(function(x){return x[0]===business;});
 var areas=b?b.slice(2):[];
 var items=areas.map(function(area){return '<span class="p6tags"><b>'+area+'</b> · '+P.filter(function(p){return p.c===area}).length+' references</span>';}).join('');
 var products=P.filter(function(p){return areas.indexOf(p.c)>-1}).slice(0,18);
 return '<div class="buildersummary"><div><span class="eyebrow darkey">PLANNING PATH</span><h2>'+business+' setup</h2><p class="muted">Start with the operating areas, then choose the equipment you actually want Twins to discuss.</p><div class="p6tags">'+items+'</div></div><div class="buildernext"><b>Next</b><span>Save the plan → build the project brief → request a quotation.</span></div></div>'+
 '<div class="sectionhead"><div><span class="eyebrow darkey">STARTING EQUIPMENT</span><h2>Build your shortlist</h2><p class="muted">These are catalogue references for planning. Exact model, specification, stock and price are confirmed with Twins.</p></div><span class="statuspill">'+products.length+' suggested references</span></div>'+
 '<div class="grid builderproducts">'+products.map(function(p){return '<label class="card buildercard"><input type="checkbox" name="equipmentIds" value="'+p.id+'" '+checked(p.id)+'><div class="cardbody"><span class="eyebrow darkey">'+p.c+'</span><h3>'+String(p.n||'Equipment').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</h3><p>'+String(p.desc||'Commercial equipment reference.').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</p><span class="muted small">ID '+p.id+' · Quote on request</span></div></label>}).join('')+'</div>';
}
document.getElementById('builder').innerHTML=head()+
'<main class="page"><section class="plannerhero"><div class="wrap"><span class="eyebrow darkey">BUSINESS EQUIPMENT PLANNER</span><h1>Plan the operation before you buy the equipment.</h1><p class="muted">Choose the business first. Twins then turns the operating areas into a practical equipment shortlist you can save, refine and take into a project quotation.</p></div></section>'+
'<section class="section"><div class="wrap"><form class="builderform" onsubmit="saveBusinessPlan(event)"><div class="panel"><div class="sectionhead"><div><span class="eyebrow darkey">01 · BUSINESS</span><h2>What are you building?</h2></div></div><div class="field"><label for="bpBusiness">Business type</label><select id="bpBusiness" required onchange="renderBusinessPlan()"><option value="">Choose a business</option>'+businessOptions+'</select></div><div class="builderfields"><div class="field"><label for="bpStage">Stage</label><select id="bpStage"><option>Idea / research</option><option>Site preparation</option><option>Equipment selection</option><option>Quotation stage</option><option>Ready to purchase</option><option>Replacing equipment</option></select></div><div class="field"><label for="bpCapacity">Expected capacity</label><input id="bpCapacity" value="'+(savedPlan?esc(String(savedPlan.capacity||'')):'')+'" placeholder="e.g. 100 meals/day, 20 trays/day"></div><div class="field"><label for="bpLocation">Location</label><input id="bpLocation" value="'+(savedPlan?esc(String(savedPlan.location||'')):'')+'" placeholder="City / state / delivery area"></div><div class="field"><label for="bpBudget">Budget guidance</label><input id="bpBudget" value="'+(savedPlan?esc(String(savedPlan.budget||'')):'')+'" placeholder="Optional range or budget note"></div><div class="field"><label for="bpUtilities">Power / fuel / water</label><input id="bpUtilities" value="'+(savedPlan?esc(String(savedPlan.utilities||'')):'')+'" placeholder="Electricity, gas, generator, water, drainage"></div><div class="field"><label for="bpSpace">Space</label><input id="bpSpace" value="'+(savedPlan?esc(String(savedPlan.space||'')):'')+'" placeholder="Approximate room size or layout note"></div></div></div><div id="builderRecommendations" class="panel builderrecommendations">'+(selected?planMarkup(selected):'<div class="empty"><h2>Choose the operation first.</h2><p class="muted">Your recommended operating areas and equipment shortlist will appear here.</p></div>')+'</div><div class="panel builderactions"><div><span class="eyebrow darkey">03 · SAVE & CONTINUE</span><h2>Keep the plan together.</h2><p class="muted">Save this business plan locally, then carry it into the detailed project brief or quotation workflow.</p></div><div class="herolinks"><button class="btn red" type="submit">Save business plan →</button><a class="btn light" href="project-planner.html">Open project planner</a><a class="btn light" href="quote.html">Request a quote</a></div></div></form></div></section></main>'+foot()+'<div id="toast"></div>';
}
function esc(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;');}
function renderBusinessPlan(){
var business=document.getElementById('bpBusiness').value,box=document.getElementById('builderRecommendations');
if(!box)return;
if(!business){box.innerHTML='<div class="empty"><h2>Choose the operation first.</h2><p class="muted">Your recommended operating areas and equipment shortlist will appear here.</p></div>';return}
var b=B.find(function(x){return x[0]===business;});var areas=b?b.slice(2):[];var products=P.filter(function(p){return areas.indexOf(p.c)>-1}).slice(0,18);
box.innerHTML='<div class="buildersummary"><div><span class="eyebrow darkey">PLANNING PATH</span><h2>'+esc(business)+' setup</h2><p class="muted">Recommended operating areas: '+areas.map(esc).join(' · ')+'</p></div><div class="buildernext"><b>Next</b><span>Choose equipment → save plan → project brief.</span></div></div><div class="sectionhead"><div><span class="eyebrow darkey">STARTING EQUIPMENT</span><h2>Build your shortlist</h2><p class="muted">Catalogue references only; confirm exact model, stock, specification and price with Twins.</p></div><span class="statuspill">'+products.length+' suggested references</span></div><div class="grid builderproducts">'+products.map(function(p){return '<label class="card buildercard"><input type="checkbox" name="equipmentIds" value="'+p.id+'"><div class="cardbody"><span class="eyebrow darkey">'+esc(p.c)+'</span><h3>'+esc(p.n)+'</h3><p>'+esc(p.desc||'Commercial equipment reference.')+'</p><span class="muted small">ID '+p.id+' · Quote on request</span></div></label>'}).join('')+'</div>';
}
function saveBusinessPlan(e){
e.preventDefault();
var business=document.getElementById('bpBusiness').value;if(!business){toast('Choose a business type');return}
var ids=Array.from(document.querySelectorAll('input[name="equipmentIds"]:checked')).map(function(x){return Number(x.value)});
var plan={type:business,stage:document.getElementById('bpStage').value,capacity:document.getElementById('bpCapacity').value,location:document.getElementById('bpLocation').value,budget:document.getElementById('bpBudget').value,utilities:document.getElementById('bpUtilities').value,space:document.getElementById('bpSpace').value,equipmentIds:ids,savedAt:new Date().toISOString()};
put('twins_business_plan',plan);
var project=get('twins_project_plan',null)||{};
if(!project.business)put('twins_project_plan',{business:business,stage:plan.stage,location:plan.location,capacity:plan.capacity,space:plan.space,utilities:plan.utilities,owned:'',needs:'Business plan shortlist: '+ids.map(function(id){var p=P.find(function(x){return x.id===id});return p?p.n:''}).filter(Boolean).join(', '),savedAt:new Date().toISOString()});
...
'<div class="field"><label>Location / delivery area</label><input id="ppLocation" value="'+(savedPlan?savedPlan.location:(savedBusiness?savedBusiness.location:''))+'" placeholder="City, state or delivery area"></div>'+
'<div class="field"><label>Expected capacity / output</label><input id="ppCapacity" value="'+(savedPlan?savedPlan.capacity:(savedBusiness?savedBusiness.capacity:''))+'" placeholder="e.g. 100 meals/day, 20 trays/day, 40 seats"></div>'+
'<div class="field"><label>Available space</label><input id="ppSpace" value="'+(savedPlan?savedPlan.space:(savedBusiness?savedBusiness.space:''))+'" placeholder="Approximate room size or layout notes"></div>'+
'<div class="field"><label>Power / fuel situation</label><input id="ppUtilities" value="'+(savedPlan?savedPlan.utilities:(savedBusiness?savedBusiness.utilities:''))+'" placeholder="Electricity, gas, generator, water, drainage"></div>'+
...
'<div class="field"><label>What do you need?</label><textarea id="ppNeeds" placeholder="Describe the equipment, workflow or menu requirements">'+(savedPlan?savedPlan.needs:(savedBusiness?'Business plan shortlist: '+(savedBusiness.equipmentIds||[]).map(function(id){var p=P.find(function(x){return x.id===id});return p?p.n:''}).filter(Boolean).join(', '):''))+'</textarea></div>'+
