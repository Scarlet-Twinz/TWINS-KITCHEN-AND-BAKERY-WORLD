...
with psycopg.connect(DATABASE_URL) as conn:
    ...
    if row: conn.execute("update users set name=%s,password_hash=%s,role='owner',updated_at=now() where id=%s",(name,password_hash,row[0]))
    else: conn.execute("insert into users (id,name,email,password_hash,role) values (%s,%s,%s,%s,'owner')",(uuid.uuid4(),name,email,password_hash))
    conn.commit()
