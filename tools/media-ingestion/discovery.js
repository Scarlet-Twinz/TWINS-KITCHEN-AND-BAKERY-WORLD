...
function rankCandidate(item){const statusRank=item?.classification?.status==='HIGH'?2:item?.classification?.status==='REVIEW'?1:0;return [statusRank,pageQuality(item?.candidate,item?.evidence,item?.match,item?.product),Number(item?.classification?.confidence||0)];}
