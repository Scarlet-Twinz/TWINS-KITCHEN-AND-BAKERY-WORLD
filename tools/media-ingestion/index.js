...
async function main(){
 ...
 if(!["dry-run","apply","audit","pending-manifest","discover"].includes(command))throw new Error("Usage: node tools/media-ingestion/index.js <audit|pending-manifest|discover|dry-run|apply> [--manifest path] [--report path]");
 if(command==="pending-manifest"){const target=arg("--output","tools/media-ingestion/manifests/pending-catalogue.json");const m=writePendingManifest(path.resolve(process.cwd(),target));console.log(JSON.stringify({output:target,...m.counts},null,2));return;}
 const data=loadCatalogue(),rules=readExistingRules(),before=computeMediaState(data,rules);
 if(command==="audit"){writeReport(buildAudit(before,{accepted:[],rejected:[],duplicateUrlsRejected:[]}));return;}
 if(command==="discover"){const proposal=await discover();writeReport(proposal);console.error("DISCOVERY ONLY: no mappings applied; data.js and app.js were not modified.");return;}
 ...
 const report=buildAudit(before,validation,after);writeReport(report);
...
main().catch(e=>{console.error("MEDIA DISCOVERY ERROR: "+e.message);process.exitCode=1;});