...
function main() {
  ...
  const before = computeMediaState(data, rules);

  if (command === "audit") {
    const report = buildAudit(before, { accepted: [], rejected: [], duplicateUrlsRejected: [] });
    writeReport(report);
    return;
  }

  const validation = validateManifest(readManifest(arg("--manifest")), before);
  let after = before;

  if (command === "apply" && validation.accepted.length) {
    applyMappings(DATA_PATH, validation.accepted);
    after = computeMediaState(loadCatalogue(), rules);
  }

  const report = buildAudit(before, validation, after);
  writeReport(report);
